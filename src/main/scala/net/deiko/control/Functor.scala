...
object Functor { 
  ...
  implicit def coproduct3Functor[F[_], G[_], H[_]](implicit F: Functor[F], G: Functor[G], H: Functor[H]) = new Functor[coproduct[F, G]#or[H]] {
    def map[A, B](ea: Either[F[A], Either[G[A], H[A]]]) = ea match { 
      case Left(fa) => Left(F.map(fa)(f))
